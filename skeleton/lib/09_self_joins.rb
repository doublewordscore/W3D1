...
def craiglockhart_and_tollcross
  ...
  execute(<<-SQL)
    ...
    JOIN
      routes AS b ON a.company = b.company AND a.num = b.num
    JOIN
      stops AS stop_a ON stop_a.id = a.stop_id
    JOIN
      stops AS stop_b ON stop_b.id = b.stop_id
    WHERE
...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    -- SELECT
    --   a.num, a.company, stop_b.name, c.num, c.company
    -- FROM
    --   routes AS a
    -- JOIN
    --   routes AS b ON a.company = b.company AND a.num = b.num
    ...
    --   routes AS c
    -- JOIN
    --   routes AS d ON c.company = d.company AND c.num = d.num
    -- JOIN
    --   stops AS stop_a ON stop_a.id = a.stop_id
    -- -- JOIN
    -- --   stops AS stop_b ON stop_b.id = b.stop_id
    -- -- JOIN
    -- --   stops AS stop_c ON stop_c.id = c.stop_id
    -- JOIN
    --   stops AS stop_d ON stop_d.id = d.stop_id
    -- WHERE
    --   (stop_a.name = 'Craiglockhart' AND stop_d.name = 'Sighthill') AND b.stop_id = c.stop_id
