...
def andrews_films_and_leads
  ...
  execute(<<-SQL)
    ...
    WHERE
      lead_castings.ord = 1 AND julie_actors.name = 'Julie Andrews'
    )
...
def films_by_cast_size
  ...
  execute(<<-SQL)
    SELECT
      movies.title, COUNT(*) AS num_cast
    FROM
      movies
    JOIN
    ...
    WHERE
      castings.movie_id in (
        ...
        FROM
          movies
        JOIN
          castings AS garf_castings ON garf_castings.movie_id = movies.id
        JOIN
          actors ON garf_castings.actor_id = actors.id
        WHERE
